...
\textbf{Note:} instead of eigenvalue-eigenvector decomposition of $\Sigma$, we can also perform singular value decomposition (SVD) of the data matrix $X$. the singular values are the square roots of the eigenvalues,  $S_i = \sqrt{\lambda_i}$.
...
Ideally, we would like to eliminate any asymptotic justification to describe this uncertainty. This is especially true for (highly) nonlinear functions of the model parameters. \textbf{Bootstrap or resampling} can be used to find the standard errors and
confidence intervals in a \textbf{frequentist framework}. \textbf{simulation-based methods like MCMC are used in a Bayesian framework}.

\subsection{Resampling and bootstrap}

Bootstrap resampling is a powerful technique that \textbf{assumes that a sample is a good representation of the population}, and we can simulate samples from the population by \textbf{sampling from the sample}. Hence, \textbf{the original sample plays the role of the population}, because \textbf{the resamples are taken from the original sample}. Therefore, \textbf{estimates from the sample play the role of true population parameters}.

\vspace{0.5em}

Each resample has the \textbf{same sample size $n$} as the original sample, \textbf{aiming to be as similar as the original sample}. The error in this process is referred to as \textit{\textbf{resampling approximation}}.

\vspace{0.5em}

\textbf{Two Basic Resampling Methods}

There are two main approaches:

\begin{itemize}
    \item \textbf{Model-free resampling} (also called \textbf{non-parametric} resampling): Resamples are \textbf{drawn with replacement from the original sample}. This ensures \textbf{independent observations}, making resamples i.i.d. like the original sample. Without replacement, every resample would be identical to the original, showing no variation.
    \item \textbf{Model-based resampling} (also called \textbf{parametric} resampling): \textbf{Assumes the original sample was drawn i.i.d.\ from a parametric density} $f(y; \theta)$. \textbf{Resamples are drawn i.i.d.} from $f(y; \hat{\theta})$, where $\hat{\theta}$ is an estimate of the parameter vector. The number of resamples $B$ should generally be large (thousands or tens of thousands).
\end{itemize}

\subsubsection{Bias, standard deviation, and MSE}

Let $\hat{\theta}$ be the estimate from the original sample and $\hat{\theta}_1^*, \ldots, \hat{\theta}_B^*$ be the estimates from the $B$ resamples. Define $\overline{\hat{\theta}^*}$ as the mean of $\hat{\theta}_1^*, \ldots, \hat{\theta}_B^*$.

\vspace{0.5em}

\textbf{Bias}

The bias is defined as $\mathbf{\text{BIAS}(\hat{\theta}) = \mathbb{E}[\hat{\theta}] - \theta}$. Since the expectation over the population is \textbf{not observable}, we \textbf{approximate} it \textbf{by averaging over resamples}:
\[
\text{BIAS}_{\text{boot}}(\hat{\theta}) = \overline{\hat{\theta}^*} - \hat{\theta}
\]

\textbf{Key insight}: \textbf{In resampling, the original sample is treated as the population, so the true parameter $\theta$ is replaced by the estimator $\hat{\theta}$} in the bootstrap estimate.

\vspace{0.5em}

\textbf{Standard Error}

The \textbf{sample principle} applies to the bootstrap standard error for $\hat{\theta}$, which is the sample standard deviation of $\hat{\theta}_1^*, \ldots, \hat{\theta}_B^*$:
\[
s_{\text{boot}}(\hat{\theta}) = \sqrt{\frac{1}{\mathbf{B-1}} \sum_{b=1}^{\mathbf{B}} (\hat{\theta}_b^* - \overline{\hat{\theta}^*})^2}
\]
...
The MSE is $\mathbb{E}[\hat{\theta} - \theta]^2$ and is estimated by:
\[
\text{MSE}_{\text{boot}}(\hat{\theta}) = \frac{1}{\mathbf{B}} \sum_{b=1}^{\mathbf{B}} (\hat{\theta}_b^* - \hat{\theta})^2
\]

\textbf{The MSE reflects both bias and variability}:
\[
\text{MSE}_{\text{boot}}(\hat{\theta}) \approx \text{BIAS}_{\text{boot}}^2(\hat{\theta}) + s_{\text{boot}}^2(\hat{\theta})
\]

\textbf{Note}: Tail parameters such as the degrees of freedom parameter for the Student-t distribution are
difficult to estimate unless the sample size is very large. With small samples, there will not be
enough extreme observations to obtain a precise estimate of the parameter.

\subsubsection{Confidence intervals}

\textbf{General Overview}

\textbf{Resampling can be used to construct confidence intervals}. The bootstrap approximates the population's probability distribution using the sample. \textbf{When a confidence interval uses an approximation, there are two coverage probabilities: the nominal one (stated) and the actual one (unknown)}. \textbf{By the accuracy of a confidence interval, we mean the degree of agreement between the nominal and actual coverage probabilities}.

\vspace{0.5em}

\textbf{Normal Approximation Interval}

For an estimate $\hat{\theta}$ with standard error $s_{\text{boot}}(\hat{\theta})$, the \textbf{normal theory confidence interval for $\theta$} is:
\[
\hat{\theta} \pm s_{\text{boot}}(\hat{\theta}) z_{\alpha/2}
\]
where $z_{\alpha/2}$ is the $\alpha/2$-upper quantile of the normal distribution.

\textbf{When $\hat{\theta}$ is an MLE, this interval is essentially the same as}:
\[
\hat{\theta} \pm s_\theta z_{\alpha/2}
\]
where $s_\theta$ is the standard error from the Fisher information matrix.

\textit{Note}: \textbf{The normal approximation does not assume the population is normally distributed, but only that $\hat{\theta}$ is normally distributed due to the central limit theorem}.

\vspace{0.5em}

\textbf{Bootstrap t-statistic Interval}

When a standard error is \textbf{available for $\hat{\theta}$} (e.g., from Fisher information), the bootstrap t-stat method offers more accurate confidence intervals. Define the \textbf{t-statistic}:
\[
t_{\text{boot},b} = \frac{\bar{Y} - \bar{Y}_{\text{boot},b}}{s_{\text{boot},b}/\sqrt{n}}
\]

The key substitutions: $\bar{Y}$ in $t$ becomes $\bar{Y}_{\text{boot},b}$; $s$ becomes $s_{\text{boot},b}$; and $\mu$ becomes $\bar{Y}$.

The resamples are i.i.d.\ and can be used to construct the distribution of the t-statistic. The \textbf{$\alpha/2$-lower and $\alpha/2$-upper quantiles} of $t_{\text{boot},b}$ values (denoted $t_L$ and $t_U$) form the bootstrap confidence interval for $\mu$:
\[
\left(\bar{Y} + t_L \frac{s}{\sqrt{n}}, \bar{Y} + t_U \frac{s}{\sqrt{n}}\right)
\]

\textbf{Challenges in Non-Normal Cases}

In the case where the distribution is not normal, we face two issues: \textbf{we do not know the distribution of the population}, and it is very difficult to obtain the distribution of the t-statistic. \textbf{Both can be overcome by resampling}.

\vspace{0.5em}

\textbf{Extension to Any Parameter}

\textbf{The method can be extended to any parameter}. Let $\hat{\theta}$ and $s(\hat{\theta})$ be the estimate of $\theta$ and its standard error calculated from the sample. Let $\hat{\theta}_b^*$ and $s_b(\hat{\theta})$ be the same quantities from the $b$-th bootstrap sample. The $b$-th bootstrap t-statistic is:
\[
t_{\text{boot},b} = \frac{\hat{\theta} - \hat{\theta}_b^*}{s_b(\hat{\theta})}
\]

Let $t_L$ and $t_U$ be the $\alpha/2$-lower and $\alpha/2$-upper sample quantiles of these t-statistics. Then the \textbf{confidence interval for $\theta$ is}:
\[
...
\textbf{Basic Bootstrap Interval}

Let $q_L$ and $q_U$ be the $\alpha/2$-lower and $\alpha/2$-upper sample quantiles of $\hat{\theta}_1^*, \ldots, \hat{\theta}_B^*$. \textbf{The fraction of bootstrap estimates that satisfy} $q_L \leq \hat{\theta}_b^* \leq q_U$ is $1 - \alpha$. This is equivalent to:
\[
\hat{\theta} - q_U \leq \hat{\theta} - \hat{\theta}_b^* \leq \hat{\theta} - q_L
\]

So that $\hat{\theta} - q_U$ and $\hat{\theta} - q_L$ are lower and upper quantiles for the distribution of $\hat{\theta} - \hat{\theta}_b^*$. The basic bootstrap interval assumes that:
\[
...
\]
\textbf{is a confidence interval for $\theta$. This interval is called the basic bootstrap interval}.

\subsubsection{Block resampling}

For time series data, \textbf{the naive resampling strategy breaks the time dependence} of the series. To preserve this dependence, \textbf{use blocks instead of single observations}. This approach is called \textbf{block resampling or block bootstrap}. 

\vspace{0.5em}

\textbf{Block Length Selection}

\textbf{A major issue is how best to select the block length}. \textbf{The correlations in the original sample are preserved only within blocks, so a large block size is desirable}. However, \textbf{the number of possible resamples depends on the number of blocks, so a large number of blocks is also desirable}. \textbf{The block can be chosen in an ad\-hoc manner, or using statistical techniques} (relying on spectral theory).



\subsection{Parametric bootstrap}
It assumes an underlying model for the data and resample from the model or the residuals of the model.

examples in the teaching notes are important. look at the notes.
...
\subsection{Goodness-of-fit principle}

\textbf{Goodness of fit} describes how well a statistical model fits a set of observations. Measures typically summarize the discrepancy between observed values and values expected under the model.

\vspace{0.5em}

The \textbf{mean-squared error (MSE)} is commonly used:
\[
\mathbf{\text{MSE}} = \frac{1}{T} \sum_{i=1}^{T} (y_i - f(x_i))^2
\]
where $f(x_i)$ is the prediction the model gives for the observation in the in-sample training data. MSE is small if predictions are close to observed data and large if they differ substantially.

\vspace{0.5em}

\textbf{The MSE is computed using the in-sample training data to fit the model and should more accurately be referred to as the training MSE}. \textbf{But in general, we need to care more about how well the method works outside the training data}. For a new out-of-sample observation $(x_0, y_0)$, \textbf{we want to know whether $f(x_0)$ is approximately equal to $y_0$}.

\vspace{0.5em}

\textbf{Training vs.\ Test MSE}

The goal is to find a method \textbf{that gives the lowest test MSE instead of the lowest training MSE}. \textbf{In general, the more flexible a method is, the lower its training MSE will be. The model will fit and explain the training data very well}.

\vspace{0.5em}

\textbf{Flexibility-Interpretability Trade-off}

More flexible methods can generate a broader range of possible shapes to estimate $f$ compared to less flexible and more restrictive methods (such as linear regression). \textbf{This represents a trade-off between flexibility and model interpretability}.

\vspace{0.5em}

\textbf{Overfitting}

\textbf{As model flexibility increases, training MSE will decrease, but the test MSE may not}. \textbf{Overfitting the data} occurs when our statistical learning procedure works too hard to find patterns in the training data, including patterns caused by random chance rather than true properties of the unknown function $f$.

\vspace{0.5em}

\textbf{Bias}

\textbf{Bias refers to the error that is introduced by modeling a real-life problem (that is usually extremely complicated) by a much simpler problem}. \textbf{The more flexible/complex a method is, the less bias it will generally have}.

\vspace{0.5em}

\textbf{Variance}

\textbf{Variance of a learning method refers to how much your estimate for $f$ would change if you had a different training data set}. A method \textbf{has high variance} if small changes in the training data result in large changes in $f$. \textbf{Generally, more flexible statistical methods have higher variance}.

\subsection{In-sample testing}

\textbf{By in-sample, we mean testing the performance of a model on the same data used for the calibration/estimation of the model}.

\vspace{0.5em}

\subsubsection{Information criteria}

One way to \textbf{measure the fit quality and penalize for large parametrization} is using information criteria. Two main criteria are the Akaike and Schwarz information criteria.

\vspace{0.5em}

\textbf{Akaike Criterion}

The \textbf{Akaike information criterion (AIC)} is an estimator of the relative quality of statistical models. \textbf{AIC estimates the relative amount of information lost by a given model: the less information a model loses, the higher the quality of that model}. AIC provides model selection based on likelihood.

\textbf{AIC deals with the trade-off between the goodness of fit of the model and the simplicity of the model}. The penalty discourages overfitting by penalizing additional parameters.

\vspace{0.3em}

For $k$ estimated parameters and $L(\hat{\theta}; y)$ as the maximum likelihood value:
\[
\mathbf{\text{AIC} = -2 \ln L(\hat{\theta}; y) + 2k}
\]

\textbf{The preferred model is the one with the minimum AIC value}. However, \textbf{AIC tells nothing about the absolute quality of a model, only the quality relative to other models}. \textbf{Thus, it is usually good practice to validate the absolute quality of the model}, including checks of residuals and tests of predictions.

\textbf{When the sample size is small, there is a substantial probability that AIC will select models that have too many parameters}. In such cases, the Schwarz information criterion can be used.

\vspace{0.5em}

\textbf{Schwarz Criterion}

\textbf{The Schwarz information criterion}, also called \textbf{Bayesian information criterion (BIC)}, is:
\[
\mathbf{\text{BIC} = -2 \ln L(\hat{\theta}; y) + k \ln T}
\]
where $k$ is the number of parameters and $T$ is the number of observations. \textbf{BIC promotes model parsimony by penalizing models with increased model complexity (larger $k$) and sample size $T$}. A model with the lowest BIC is preferred.

\vspace{0.5em}

\textbf{Deviance Information Criterion}

\textbf{The DIC consists of two terms: a component that measures the goodness-of-fit and a penalty term for any increase in model complexity}. \textbf{The measure of fit is obtained by taking the posterior expectation of the Deviance}:
\[
\bar{\mathbf{D}} = \mathbb{E}_{p(\theta|y)}[D(\theta)]
\]
where $D(\theta) = -2 \ln L(y|\theta)$ is the Deviance. This can be approximated using the posterior sample.

\vspace{0.3em}

\textbf{The second component measures the complexity of the model using the effective number of parameters} $p_D$, defined as the difference between the posterior mean Deviance and the Deviance at a point estimate $\hat{\theta}$:
\[
p_D = \bar{D} - D(\hat{\theta})
\]

\textbf{A natural candidate for $\hat{\theta}$ is the posterior mean}. The DIC is then:
\[
\mathbf{\text{DIC}} = \bar{D} + p_D
\]
\textbf{The model with the smallest DIC has the best balance between goodness-of-fit and model complexity}.

\vspace{0.5em}

\subsubsection{Probability integral transform}

\textbf{We can analyze the predictive probabilities referred to as probability integral transforms or p-scores}. Using a point estimate $\hat{\theta}$ (maximum likelihood or posterior mean), the p-scores are:
\[
z_t = \mathbf{F(y_t; \hat{\theta})} \quad t = 1, \ldots, T
\]
where $y_t$ is the observation at time $t$ and $F$ is the cumulative distribution function of the model. \textbf{Under a correct specification, the p-scores should have independent uniform distributions}.

\subsection{Out-of-sample testing}

\textbf{By out-of-sample, we mean testing the performance of a model on different data than one used for the calibration/fitting of the model}.

\vspace{0.5em}

\subsubsection{Information criteria}

\textbf{We can compute the likelihood out-of-sample and thus compute the various criteria in an out-of-sample context}:
\[
L(\bar{y}_1, \ldots, \bar{y}_T; \hat{\theta})
\]
where $\hat{\theta}$ is estimated using $y_1, \ldots, y_T$, and $\bar{y}_1, \ldots, \bar{y}_T$ is the new out-of-sample dataset.

\vspace{0.5em}

\subsubsection{Probability integral transform}

\textbf{The probability integral transform can be used as well out-of-sample}. \textbf{For instance, we can backtest a GARCH model, and then investigate the out-of-sample p-scores}, computed iteratively on various rolling windows.

\vspace{0.5em}

\subsubsection{Value-at-risk tests}

\textbf{For testing the accuracy of the VaR predictions, we use the so-called hit variable, which is a dummy variable indicating a loss that exceeds the VaR level}:
\[
I_t^* = I(y_t \leq \text{VaR}_t^\alpha)
\]

where $\text{VaR}_t^\alpha$ denotes the VaR prediction at risk level $\alpha$ and $I(\cdot)$ is the indicator function. \textbf{If the VaR is correctly specified, the hit variable has a mean value of $\alpha$ and is independently distributed over time}. Standard tests include the unconditional coverage (UC) test, conditional coverage (CC) test, and dynamic quantile (DQ) test.
